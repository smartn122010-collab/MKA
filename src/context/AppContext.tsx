...
import { seedDatabaseIfNeeded, INITIAL_PRODUCTS, INITIAL_OFFERS, INITIAL_STATS } from '../utils/seeder';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Helper to ensure each product / offer has a unique ID in demo mode
  const ensureProductIds = (list: Product[]): Product[] => {
    return list.map((p, idx) => ({
      ...p,
      id: p.id || `local_prod_${idx + 1}`
    }));
  };

  const ensureOfferIds = (list: Offer[]): Offer[] => {
    return list.map((o, idx) => ({
      ...o,
      id: o.id || `local_offer_${idx + 1}`
    }));
  };
  ...
  const enterDemoMode = (role: 'admin' | 'customer') => {
    ...
    if (!localStorage.getItem('mka_local_products')) {
      localStorage.setItem('mka_local_products', JSON.stringify(ensureProductIds(INITIAL_PRODUCTS)));
    }
    if (!localStorage.getItem('mka_local_offers')) {
      localStorage.setItem('mka_local_offers', JSON.stringify(ensureOfferIds(INITIAL_OFFERS)));
    }
    ...
    const rawLocalProducts = JSON.parse(localStorage.getItem('mka_local_products') || '[]');
    const rawLocalOffers = JSON.parse(localStorage.getItem('mka_local_offers') || '[]');
    setProducts(ensureProductIds(rawLocalProducts));
    setOffers(ensureOfferIds(rawLocalOffers));
  ...
  useEffect(() => {
    if (isDemoMode) {
      ...
      const parsedOffers = savedOffers ? JSON.parse(savedOffers) : INITIAL_OFFERS;
      
      setProducts(ensureProductIds(parsedProducts));
      setOffers(ensureOfferIds(parsedOffers));
